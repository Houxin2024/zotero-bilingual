...
- 智能常驻页面缓存：双语 PDF 最近访问的 64 个页面只有在完整渲染后才会保留普通画布、超宽页的高清 detail canvas 与渲染状态；渲染中的页面始终允许 PDF.js 取消并重试，避免空白页卡死。即使 Zotero 重建内部阅读器实例也会自动重新接管，关闭标签后自动释放。
...
常驻缓存默认保留最近 64 个已访问页面。可在 Zotero 高级配置编辑器中修改
`extensions.bilingualLinkedReader.residentPageLimit`（支持 10–128）；缓存仅在双语 PDF 标签存活期间占用内存。
