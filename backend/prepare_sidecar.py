...
def prepare(
    ...
    if geometry_only:
        ...
        return {"version": 2, **geometry["stats"]}
    try:
        stats = realign(temporary, output, model, cache_dir)
        temporary.unlink(missing_ok=True)
        return {"version": 3, **stats}
    except Exception as error:
        os.replace(temporary, output)
        return {"version": 2, "semanticError": str(error), **geometry["stats"]}


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--original", type=Path, required=True)
    parser.add_argument("--translated", type=Path, required=True, help="Chinese mono PDF")
