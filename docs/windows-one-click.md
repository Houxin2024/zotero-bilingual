...
翻译过程中会在 Zotero 主窗口内显示可关闭的阶段卡片。它不会把每个内部子阶段都显示为整篇 100%，关闭卡片也不会中断翻译。

升级用户也需要重新安装这两个 XPI；即使已经安装官方 PDF2zh 4.0.3，也请安装包内的 `4.0.3.3` 本地修订版，进度修复和新状态卡包含在其中。
...
首次安装需要联网下载 Python 依赖、字体及语义模型。论文 PDF、翻译文本和 API Key 不会发送到本项目的服务器；用户自行选择的翻译提供商可能有不同的数据政策。
